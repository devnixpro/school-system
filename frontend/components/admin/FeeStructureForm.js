    ...
    <form onSubmit={submit} className="space-y-6">
      <div>
        <div className="flex items-center justify-between mb-3">
          <label className="text-sm font-semibold text-gray-900">Students <span className="ml-2 text-xs font-normal text-gray-500">{form.studentIds.length} selected</span></label>
          {filtered.length > 0 && (
        ...
        <input type="search" placeholder="Search by name or roll no…" value={query} onChange={(e) => setQuery(e.target.value)}
        ...
        <div className="max-h-64 overflow-y-auto rounded-xl border border-gray-200 divide-y divide-gray-100">
          ...
          {!loading && filtered.length === 0 && (
            <p className="p-4 text-sm text-gray-400">{students.length === 0 ? "No students yet." : "No students match your search."}</p>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <input type="number" min="0" placeholder="5000" value={form.amount} onChange={set("amount")} required className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm" />
        </div>
        <div>
          <label className="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Month</label>
          <input type="text" value={form.month} onChange={set("month")} required className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm" />
        </div>
        <div>
          <label className="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Due date</label>
          <input type="date" value={form.dueDate} onChange={set("dueDate")} required className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm" />
        </div>
      </div>

      {form.studentIds.length > 0 && form.amount && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center justify-between">
          <span className="text-sm text-blue-700">{form.studentIds.length} invoice{form.studentIds.length !== 1 ? "s" : ""} × PKR {Number(form.amount).toLocaleString()}</span>
          <span className="font-bold text-lg text-blue-900">PKR {total.toLocaleString()}</span>
        </div>
      )}

      <button type="submit" disabled={!form.studentIds.length || !form.amount || submitting}
