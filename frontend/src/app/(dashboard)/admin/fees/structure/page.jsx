...
export default function FeeStructurePage() {
  ...
  const submit = async (payload) => {
    try {
      ...
      setRecent(res.data || []);
      if (confirm(`${res.count} invoices created! View them now?`)) {
  ...
  return (
    <main className="p-6 lg:p-8 max-w-4xl mx-auto space-y-6">
      <header>
        <h1 className="font-poppins text-3xl font-bold text-navy-900">Generate Fee Invoices</h1>
        <p className="text-navy-500 text-sm mt-1">
          Select students, set an amount and due date — invoices are created instantly.
        </p>
      </header>
      ...
      {recent.length > 0 && (
        <div className="bg-white rounded-2xl border border-navy-100 p-6">
          <h3 className="font-poppins font-semibold text-navy-900 mb-4">Just Created ({recent.length})</h3>
          <ul className="text-sm divide-y divide-navy-100">
            {recent.slice(0, 5).map((fee) => (
              <li key={fee._id} className="py-2 flex justify-between">
                <span className="font-mono text-xs text-navy-500">{fee.invoiceNo}</span>
                <span className="text-navy-700 font-medium">PKR {Number(fee.amount).toLocaleString()}</span>
              </li>
            ))}
          </ul>
        </div>
