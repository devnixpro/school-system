...
export default function ChildFeesPage() {
  ...
  return (
    <main className="p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
      <header>
        <h1 className="font-poppins text-3xl font-bold text-navy-900">Child Fees</h1>
        <p className="text-navy-500 text-sm mt-1">Enter your child&apos;s student ID to view their fee records.</p>
      </header>

      <input type="text" placeholder="Student ID" value={childId}
        onChange={(e) => setChildId(e.target.value.trim())}
        className="px-4 py-3 border border-navy-200 rounded-xl text-sm w-full max-w-sm outline-none focus:ring-2 focus:ring-brand-200 focus:border-brand" />
        ...
        <>
          <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="rounded-2xl border bg-green-50 border-green-200 p-5">
              <div className="text-[11px] font-semibold uppercase tracking-wider text-green-700">Paid</div>
              <div className="font-poppins text-2xl font-bold text-green-900 mt-2">{paid.length}</div>
              <div className="text-xs text-navy-500 mt-1">Invoices</div>
            </div>
      ...
      {!loading && childId && fees.length === 0 && (
        <div className="bg-white rounded-2xl border border-navy-100 py-16 text-center">
          ...
          <h3 className="font-poppins font-semibold text-lg text-navy-900 mb-1">No fees found</h3>
          <p className="text-sm text-navy-500">Double-check the student ID and try again.</p>
        </div>
      )}
      ...
      {!loading && !childId && (
        <div className="bg-white rounded-2xl border border-navy-100 py-16 text-center">
          <div className="text-6xl mb-4">👶</div>
          <h3 className="font-poppins font-semibold text-lg text-navy-900 mb-1">Enter a student ID</h3>
          <p className="text-sm text-navy-500">Ask your child&apos;s school for the ID if you don&apos;t have it.</p>
        </div>
      )}
    </main>
  );
